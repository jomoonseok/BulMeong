...
public class RequiredLoginAspect {
	...
	@Pointcut("execution(* com.gdu.bulmeong.*.controller.*Controller.requiredLoginAjax_*(..))")
	public void requiredLoginAjax() { }
	
	@Before("requiredLoginAjax()")  // 포인트컷 실행 전에 requiredLogin() 메소드 수행
	public Map<String, Object> requiredLoginAjaxHandler(JoinPoint joinPoint) throws Throwable {
...
//		}
		...
		if(session.getAttribute("loginUser") == null) {		
			...
			map.put("message", "<script>alert('로그인이 필요한 기능입니다')</script>");
			PrintWriter out = response.getWriter();	
